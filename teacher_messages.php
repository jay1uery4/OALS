...
$inbox_query = "SELECT `message_id`, `reciever_id`, `content`, `date_sended`, `sender_id`, `reciever_name`, `sender_name`, `message_status` FROM `message` WHERE `reciever_id` = 'current_user_id'"; // Adjust the condition as per your logic
$inbox_result = mysqli_query($conn, $inbox_query);

// Fetch sent messages
$sent_query = "SELECT `message_sent_id`, `reciever_id`, `content`, `date_sended`, `sender_id`, `reciever_name`, `sender_name` FROM `message_sent` WHERE `sender_id` = 'current_user_id'"; // Adjust the condition as per your logic
...
<div id="pgside" class="drawer">
    ...
    <a href="teacher_messages.php" class="sidebar-btn active"><i class="ico fa fa-chalkboard-teacher"></i><span class="txt">Messages</span></a>
...
<main id="pgmain">
        ...
        <div class="student-dashboard">
            <div class="dashboard-card">
                <div class="card-content">
                    ...
                    <button class="create-message-btn"><i class="fas fa-pencil-alt"></i> Create Message</button>
                </div>
                    <div class="tabs">
                        <button class="tab-button active" onclick="showTab('inbox')">Inbox</button>
                        <button class="tab-button" onclick="showTab('sent')">Sent Messages</button>
                    </div>
                    <!-- Include Inbox and Sent Messages -->
        ...
        <div id="createMessages" class="createMessagesmodal" style="display: none;">
            <div class="createMessagesmodal-content">
                <span class="close" onclick="closeCreateMessages()">&times;</span>
                <h2>Create Message</h2>
                <div class="tabs">
                    <button class="tab-button active" onclick="showComposeTab('students')">For Students</button>
                    <button class="tab-button" onclick="showComposeTab('teachers')">For Teachers</button>
                </div>
                <!-- Content for Students -->
                <div id="students" class="compose-tab active">
                    <!-- Student selection -->
                </div>
                <!-- Content for Teachers -->
                <div id="teachers" class="compose-tab">
                    <!-- Teacher selection -->
                </div>
                <label for="message-content">Message:</label>
                <textarea id="message-content" rows="4" placeholder="Type your message here..."></textarea>
                <button id="send-message"><i class="fas fa-paper-plane"></i> Send Message</button>
            </div>
        </div>
</main>
